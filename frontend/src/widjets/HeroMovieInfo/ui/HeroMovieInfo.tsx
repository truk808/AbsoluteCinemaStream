...
export const HeroMovieInfo = () => {
    const {film, trailer, isLoading, error} = useHeroMovieInfo()

    if (!film) return <div className='w-full h-full text-white text-9xl felx justify-center items-center'>фильма нет! :(</div>;

    if (isLoading) return <svg class="size-8 animate-spin text-brand-text" xmlns="http://w3.org" fill="none" viewBox="0 0 24 24">
